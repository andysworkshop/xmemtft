...
namespace lcd {
  ...
  inline void ILI9327Colour<COLOURS_16BIT,TAccessMode>::setColourMode() const {
    TAccessMode::writeCommand(ili9327::SET_PIXEL_FORMAT);
    TAccessMode::writeData(ili9327::COLOURS_64K);
  }
  ...
  template<class TAccessMode>
  inline void ILI9327Colour<COLOURS_16BIT,TAccessMode>::rawFlashTransfer(uint32_t buffer,uint32_t numPixels) const {
  ...
  template<class TAccessMode>
  inline uint8_t ILI9327Colour<COLOURS_16BIT,TAccessMode>::getBytesPerPixel() const {
    return 2;
  }
}
