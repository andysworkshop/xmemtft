...
/**
 * @file R61523Colour24Specialisation.h
 * @brief Specialisation of R61523Colour for 24-bit colours.
 * @ingroup R61523
 */
...
namespace lcd {
  ...
  template<class TAccessMode>
  inline void R61523Colour<COLOURS_24BIT,TAccessMode>::rawFlashTransfer(uint32_t data,uint32_t numPixels) const {

    TAccessMode::writeCommand(r61523::MEMORY_WRITE);
    TAccessMode::rawTransfer(data,numPixels*2);
  }
  ...
  template<class TAccessMode>
  inline uint8_t R61523Colour<COLOURS_24BIT,TAccessMode>::getBytesPerPixel() const {
    return 4;
  }
}
