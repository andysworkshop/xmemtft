...
namespace lcd {
	...
	inline void ILI9325Orientation<LANDSCAPE,TAccessMode>::moveTo(const Rectangle& rc) const {
		TAccessMode::writeCommandData(ILI932X_HOR_START_AD,rc.Y);
		TAccessMode::writeCommandData(ILI932X_HOR_END_AD,(rc.Y + rc.Height - 1));

		TAccessMode::writeCommandData(ILI932X_VER_END_AD,(319 - rc.X));
		TAccessMode::writeCommandData(ILI932X_VER_START_AD,(319 - (rc.X + rc.Width - 1)));

		TAccessMode::writeCommandData(ILI932X_GRAM_HOR_AD,rc.Y);
		TAccessMode::writeCommandData(ILI932X_GRAM_VER_AD,(319 - rc.X));
	}
