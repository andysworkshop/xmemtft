...
typedef NokiaN93_Portrait_262K LcdAccess;
LcdAccess *tft;
DefaultBacklight *backlight;


void weather(uint32_t pixelData,uint32_t pixelDataSize,const Size& size,uint16_t startingTemp);
void showSummary(uint32_t pixelData,uint32_t pixelDataSize,const Size& size);
void showTemperature(uint16_t temperature);
void adjustTemperature(uint16_t& temperature);
void writeBitmap(const BitmapInfo *bmInfo,const Point& position);
...
void showSummary(uint32_t pixelData,uint32_t pixelDataSize,const Size& size) {
	...
	lastSummaryRect.X=(240-size.Width)/2;
...
void adjustTemperature(uint16_t& temperature) {
	...
	// adjust the temperature by +/- .1 or +/- .2
